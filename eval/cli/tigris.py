...
def object_info(key, bucket):
    ...
    try:
        ...
        # Generate public URL
        endpoint = os.getenv("AWS_ENDPOINT_URL_S3", "https://fly.storage.tigris.dev").replace(
            "https://", ""
        )
    ...
    try:
        ...
        with open(local_path, "rb") as f:
            s3_client.upload_fileobj(
                f, bucket_name, key, ExtraArgs={"ContentType": content_type, "ACL": "public-read"}
            )
...
def download_file(key, local_path, bucket, overwrite):
    ...
    try:
        ...
        def progress_callback(bytes_transferred):
            ...
            click.echo(
                f"\r   Progress: {progress:.1f}% ({bytes_transferred / (1024 * 1024):.2f} MB)",
                nl=False,
            )

        with open(local_path, "wb") as f:
            s3_client.download_fileobj(bucket_name, key, f, Callback=progress_callback)
...
def bucket_info(bucket):
    ...
    try:
        ...
        click.echo(
            f"   Endpoint: {os.getenv('AWS_ENDPOINT_URL_S3', 'https://fly.storage.tigris.dev')}"
        )
...
@click.option("--bucket", "-b", help="Bucket name (uses env TIGRIS_BUCKET_NAME if not specified)")
@click.option(
    "--dry-run", "-n", is_flag=True, help="Show what would be deleted without actually deleting"
)
