...
@click.option(
    "--status",
    "-s",
    type=click.Choice(["draft", "active", "completed", "archived"]),
...
@click.confirmation_option(
    prompt="⚠️  Are you sure you want to permanently delete this experiment and ALL its data?"
)
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            for table in tables:
                if table == "Experiment":
                    cursor.execute(
                        f'DELETE FROM "{table}" WHERE id = %s OR slug = %s',
                        (experiment_id, experiment_id),
                    )
                else:
                    cursor.execute(
                        f'DELETE FROM "{table}" WHERE "experimentId" = %s', (experiment_id,)
                    )
