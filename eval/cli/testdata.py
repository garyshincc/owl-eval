    ...
    try:
        ...
        if not dry_run:
            ...
            framework = ABTestingFramework(
                experiment_name=experiment_name, experiment_description=experiment_description
            )
        ...
        click.echo(
            f"✅ {'Loaded' if not dry_run else 'Would load'} {len(models)} models: {list(models.keys())}"
        )
        ...
        model_pairs = [
            (m1, m2) for i, m1 in enumerate(models.keys()) for m2 in list(models.keys())[i + 1 :]
        ]
        total_comparisons = len(test_scenarios) * len(model_pairs) * pairs_per_scenario

        click.echo(
            f"📊 Plan: {len(test_scenarios)} scenarios × {len(model_pairs)} pairs × {pairs_per_scenario} reps = {total_comparisons} comparisons"
        )
...
@click.option(
    "--data-dir", "-d", required=True, help="Path to data directory (e.g., data/test_run)"
)
@click.option("--experiment-name", "-n", required=True, help="Name for the imported experiment")
@click.option(
    "--experiment-description", "-desc", default="", help="Description for the imported experiment"
)
@click.option(
    "--upload-videos/--no-upload-videos", default=True, help="Upload local videos to Tigris"
)
            ...
            for comparison_data in bar:
                ...
                if upload_videos:
                    ...
                    video_a_path = os.path.join(
                        data_dir, comparison_data["model_a_video_path"].replace("./", "")
                    )
                    video_b_path = os.path.join(
                        data_dir, comparison_data["model_b_video_path"].replace("./", "")
                    )
...
def _create_mock_models(dry_run: bool = False) -> Dict:
    ...
    if dry_run:
        return {"Skywork/Matrix-Game": None, "baseline_model": None}
